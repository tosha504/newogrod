...
{
	register_taxonomy('knoweledge_categories', 'knoweledge_base', array(
		'hierarchical'  	=> true,
		'label' 					=> 'Kategorie',
		'show_in_rest' 		=> true,
		'show_ui'       	=> true,
		'rewrite' 				=> array('slug' => 'kategorie', 'with_front' => true)
	));
	...
	register_taxonomy('garden_categories', 'garden_departments', array(
		...
		'show_in_rest' 		=> true,
		'show_ui'       	=> true,
		'rewrite' 				=> array('slug' => 'dzialy', 'with_front' => true)
	));
}
...
function show_archive_top()
{
	...
	<div class="page-banner" <?php if ($banner_page) {
															echo 'style="background-image: linear-gradient(0deg, rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)),url(' . wp_get_attachment_image_url($banner_page, 'full') . '); "';
														} ?>>
		<div class="container">
			<?php
			if (is_home() || is_tax('category-course') || is_page('wydarzenia')) { ?>
				<div class="choose">
					<a class="course<?php echo is_tax('category-course') ? ' active' : ''; ?>" href="<?php echo get_site_url(); ?>/rodzaj-oferty/edukacja/">
					...
					<a class="offer-blog <?php echo is_home() ? ' active' : ''; ?>" href="<?php echo get_site_url(); ?>/oferta/">
						...
						<p class="choose__title">Aktualności</p>
						<span class="link-arrow-green"><?php _e('Read more', 'garden') ?></span>
					</a>
				</div>
			<?php } ?>
...
add_image_size('custom-small-square', 150, 150, true);
