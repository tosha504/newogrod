...
if (!function_exists('garden_scripts')) {
  ...
  function garden_scripts()
  {
    ...
    //Slick	slider 
    wp_enqueue_script('slick_theme_functions', $theme_uri . '/libery/slick.min.js', ['jquery'], false, true);
    // Custom JS
  ...
  if (isset($_POST['tag']) && !empty($_POST['tag'])) {
    ...
    if ($query->have_posts()) {
      ...
      while ($query->have_posts()) {
        $query->the_post();
        $terms = wp_get_post_terms(get_the_ID(), array('knoweledge_categories', 'category-course'));
